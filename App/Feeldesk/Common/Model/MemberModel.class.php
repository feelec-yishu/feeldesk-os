...
class MemberModel extends BasicModel
{
    ...
    public function checkMemberByField($field = '',$value = '',$member_id = 0,$isCheckClosed=false)
    {
        ...
        $member_id = $this->where($where)->getField('member_id');

        return $member_id;
    }


    public function VerifyCreateData($member = [],$type = 0,$company_id = 0)
    {
    ...
    public function VerifyEditorData($member=[],$member_id=0,$firm_id=0)
    {
        ...
        if($member['email'])
        {
            ...
            if($this->checkMemberByField('account',$member['email'],$member_id)) return ['status'=>0,'msg'=>L('MAIL_EXISTS')];

            if($this->checkMemberByField('email',$member['email'],$member_id)) return ['status'=>0,'msg'=>L('MAIL_EXISTS')];
        }
        ...
        if($member['mobile'])
        {
            ...
            if($this->checkMemberByField('account',$member['mobile'],$member_id))
            {
	            return ['status'=>0,'msg'=>L('MOBILE_EXISTS')];
            }

            if($this->checkMemberByField('mobile',$member['mobile'],$member_id))
            {
	            return ['status'=>0,'msg'=>L('MOBILE_EXISTS')];
            }
        }
		...
		if($member['wechat'])
        {
            if($this->checkMemberByField('wechat',$member['wechat'],$member_id)) return ['status'=>0,'msg'=>L('WECHAT_NUMBER_EXISTS')];
        }
	...
	*/
    public function deleteCustomer($id,$company_id)
    {
        ...
        {
            $result = ['status'=>2,'msg'=>L('UPDATE_SUCCESS'),'url'=>U(MODULE_NAME.'/'.CONTROLLER_NAME.'/index')];
        }
        else
        {
...
//				  创建默认角色、权限
            ...
            $result = ['status'=>1,'msg'=>L('UPDATE_SUCCESS'),'url'=>U(MODULE_NAME.'/'.CONTROLLER_NAME.'/visitor_list')];
        }
        else
        {
            $result = ['status'=>0,'msg'=>L('UPDATE_FAILED')];
...
//        删除与用户相关的数据
        $tableArr = [
        	'member',//用户
	        'ticket_reply',//用户回复
	        'reply_comment',//用户评论回复
	        'team_reply',//用户内部协作回复
	        'ticket_event',//用户相关的工单事件
	        'cc_ticket',//用户是抄送人
	        'ticket_satisfy',//用户的满意度评价
	        'ticket_partake',//用户参与的工单
        ];
...
//        条件 - 删除与用户相关的工单数据
	    $ticketWhere = ['member_id|recipient_id|dispose_id'=>['in',$id]];

        $ticketIds = M('ticket')->where($ticketWhere)->field('ticket_id')->select();

	    $ticketIds = array_column($ticketIds,'ticket_id');

	    foreach($ticketIds as $t)
	    {
		    D('Ticket')->deleteTicket($company_id,$t);
	    }

        if($i == count($tableArr))
        {
            $result = ['status'=>2,'msg'=>L('DELETE_SUCCESS'),'url'=>U(MODULE_NAME.'/'.CONTROLLER_NAME.'/index')];
        }
        else
        {
    ...
    public function updatePassword($member,$member_id,$company_id,$type)
    {
        ...
        {
            return ['status'=>2,'msg'=>L('UPDATE_SUCCESS'),'url'=>U(MODULE_NAME.'/'.CONTROLLER_NAME.'/userinfo')];
        }
    }
