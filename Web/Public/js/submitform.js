...
$(function()
{
    ...
    }).use(['form','upload','element','layedit','layUploader'], function()
    {
		...
		followEditor = layedit.build('follow_content',{tool: ['strong','italic' ,'underline' ,'del' ,'|' ,'left' ,'center','right'], height:200}); //建立编辑器

		$('#layuiEditor,#layuiEditorEn,#layuiEditorJp,#fastEditor,#tempEditor,#follow_content').next('.layui-layedit').find('iframe').contents().find('body').unbind('paste').bind('paste',function(e)
		{
			//剪切板
			var clipboardData = event.clipboardData || window.clipboardData || event.originalEvent.clipboardData;
