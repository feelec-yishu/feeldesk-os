...
class Email
{
	...
	*/
	public function mailConnect()
	{
		...
		if (!$this->_connect)
		{
			$message = "邮箱【".$this->account."】连接失败。失败原因：" . imap_last_error();

			Log::write($message,'INFO','File',LOG_PATH.'Mail/mailbox-error.log');
	...
	public function mailReceived()
	{
		...
		else
		{
			...
			foreach ($attachments as $attach)
			{
				...
				elseif ($encode == 'CP936')
				{
					$file_name = iconv('UTF-8','latin1//IGNORE', $file_name);
				}
				...
				// '附件' 保存到服务器 以供下载
				else if(in_array($attach['type'],[0,3,5,6]) && !$attach['inline'])
				{
					// 重命名附件，避免覆盖已存在的同名附件
	...
	* @return bool|array
	*/
	public function getHeader(string $mailId)
	{
...
//		die;
	...
	* @return string
	*/
	public function getBody(string $mailId)
	{
	...
	private function getPart($mailId,$mimeType,$structure=false,$partNumber=false,$base64=true)
	{
		...
		if ($structure)
		{
			if ($mimeType == $this->getMimeType($structure))
			{
				...
				$codes = ["ASCII",'UTF-8','GB2312','GBK','GB18030',"EUC-JP",'EUC-KR','EUC-CN','BIG5','KOI8-R','ISO-8859-1','ISO-8859-5','CP936','DBCS','UCS','ANSI','JIS','SJIS','UCS-2LE','UTF-7','EUCJP-WIN', 'SJIS-WIN'];

				$encode = mb_detect_encoding($text, $codes);
				...
				// 非utf-8编码需要转码
				if(($encode != 'UTF-8' || ($fromEncoding != 'UTF-8' && in_array(strtoupper($fromEncoding),$codes))) && $base64)
				{
					if($encode == 'CP936')
					{
						$text = iconv('UTF-8','latin1//IGNORE', $text);
					}
					else
					{
