	...
	public function _initialize()
	{
		...
		if($this->_company_id > 0)
		{
			$this->_config = [
				...
				'savePath'   => 'Uploads/',
				'saveName'   => ['uniqid',''],
				'exts'       => ['jpeg', 'png', 'gif', 'jpg','zip', 'rar', 'txt', 'doc', 'docx', 'xlsx', 'xls', 'pptx', 'pdf', 'chf'],
				'autoSub'    => true,
				'subName'    => ['date','Ymd'],
			];
...
//    上传头像
	...
	* @param string $space 储存空间 local 本地储存
	* @param string $from
	...
	*/
	public function deleteUploadFile($file,$from = '')
	{
		...
		$protocol = strpos($file,'https') ? 'https' : 'http';

		$file = str_replace($protocol.'://'.C('HOST_DOMAIN'),'',$file);
	...
	public function deleteTicketFile($files = '')
	{
		...
		foreach($images as $v)
		{
			...
			$protocol = strpos($v,'https') ? 'https' : 'http';

			$v = str_replace($protocol.'://'.C('HOST_DOMAIN'),'',$v);
	...
	public function delEditImage($img=[],$old_img='')
	{
		if($old_img)
		{
			...
			foreach($old_img as $v)
			{
				if(!in_array($v,$img))
				{
					$img_name = getImageName("http://".$this->_config['driverConfig']['domain']."/",$v);
...
//    删除时，删除富文本中的图片
	public function delImage($content='')
	{
		...
		if($img[1])
		{
			foreach($img[1] as $v1)
			{
				$img_name = getImageName("http://".$this->_config['driverConfig']['domain']."/",$v1);
