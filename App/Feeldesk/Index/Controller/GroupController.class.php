...
{
	protected static $_filter = ['group_name','manager_id','ticket_auth','orderby','is_default','closed'];
...
//    部门列表
	public function index($request = '',$page = 300,$keyword = '')
	{
        ...
        else
        {
	        ...
	        foreach($ids as $val)
	        {
		        $parentIds = $this->getParentIds($departments,$val['group_id']);
...
//				更新子级层级
				$this->updateChildLevel($group_id,$data['level']);
		...
		else
		{
			...
			$selected = $this->getParentIds($departments,$group_id);
	...
	private function checkRequestData($source = '',$group_id = 0)
	{
		...
		$parent_id = explode(',',trim($parent_id,','));
		...
		if($source == 'editor')
		{
			...
			{
//			    父级部门的父级
				$pid = M('group')->where(['company_id'=>$this->_company_id,'group_id'=>$parent_id])->getField('parent_id');

				if(isset($pid) && $pid == $group_id)
				{
...
//				默认展开的值
			...
			if($v['parent_id'] == $parent_id)
			{
				...
				$face = $v['manager']['face'] ?: '/Attachs/default.jpg';
	...
	private function getParentIds($data,$id,$level = 1)
	{
	...
	*/
	function updateChildLevel($parent_id,$level)
	{
		$function = __FUNCTION__;
